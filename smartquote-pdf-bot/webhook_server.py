...
def lark_ocr_image(image_path):
    """Use Lark OCR API to extract text from image"""
    try:
        import base64
        token = get_tenant_token()
        ...
        print(f"[LARK OCR] Image base64 size: {len(image_data)} chars")

        # Lark OCR API - correct endpoint
        resp = httpx.post(
        ...
        data = resp.json()
        print(f"[LARK OCR] Response code: {data.get('code')}, msg: {data.get('msg', 'N/A')}")
        if data.get("code") == 0:
            text = data.get("data", {}).get("text", "")
            print(f"[LARK OCR] Extracted {len(text)} chars")
            return text
        else:
            print(f"[LARK OCR] API error: {data}")
    except Exception as e:
        print(f"[LARK OCR] Failed: {type(e).__name__}: {e}")
    return ""
...
def extract_pdf_text(pdf_path):
    ...
    print("[PDF] === No text found, starting OCR fallback ===")
    try:
        import pymupdf
...
def process_pdf(pdf_path):
    ...
    print(f"[PDF] extract_pdf_text returned {len(text.strip())} chars, {page_count} pages")
...
async def handle_message(event):
    ...
    print(f"[LARK] content={json.dumps(content, ensure_ascii=False)[:500]}")
...
# ── Test PDF Endpoint (for debugging OCR) ──────────────────────────────────
@app.post("/test-pdf")
async def test_pdf(request: Request):
    """Test PDF OCR processing - upload a PDF file to test"""
    import traceback
    try:
        form = await request.form()
        file = form.get("file")
        if not file:
            return JSONResponse({"error": "No file uploaded. Use multipart form with 'file' field."}, status_code=400)
        ...
        log = []
        log.append(f"PDF saved: {pdf_path}, size={len(content_bytes)} bytes")
        ...
        try:
            ...
            log.append(f"pdftotext: returncode={result.returncode}, stdout_len={len(result.stdout.strip())}")
            if result.stdout.strip():
                log.append(f"pdftotext result (first 500 chars): {result.stdout[:500]}")
        except FileNotFoundError:
            log.append("pdftotext: NOT FOUND (not installed)")
        except Exception as e:
            log.append(f"pdftotext FAILED: {e}")
        ...
        try:
            ...
            for page in reader.pages:
                ...
                if t:
                    texts.append(t)
            text = "\n".join(texts)
            log.append(f"PyPDF2: pages={len(reader.pages)}, text_len={len(text.strip())}")
            if text.strip():
                log.append(f"PyPDF2 result (first 500 chars): {text[:500]}")
        except Exception as e:
            log.append(f"PyPDF2 FAILED: {e}")
        
        # Step 3: pymupdf + OCR
        try:
            import pymupdf
            doc = pymupdf.open(str(pdf_path))
            log.append(f"pymupdf: pages={len(doc)}")
            for page_num, page in enumerate(doc):
                pix = page.get_pixmap(matrix=pymupdf.Matrix(2, 2))
                with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
                    pix.save(tmp.name)
                    tmp_path = tmp.name
                log.append(f"Page {page_num+1}: pixmap saved, size={pix.width}x{pix.height}")
                ...
                try:
                    ...
                    img_pil = Image.open(tmp_path)
                    text = pytesseract.image_to_string(img_pil, lang='chi_tra+eng')
                    log.append(f"pytesseract: text_len={len(text.strip())}")
                    if text.strip():
                        log.append(f"pytesseract result (first 500 chars): {text[:500]}")
                except ImportError:
                    log.append("pytesseract: NOT INSTALLED")
                except Exception as e:
                    log.append(f"pytesseract FAILED: {type(e).__name__}: {e}")
                
                # Try Lark OCR
                try:
                    lark_text = lark_ocr_image(tmp_path)
                    log.append(f"Lark OCR: text_len={len(lark_text.strip())}")
                    if lark_text.strip():
                        log.append(f"Lark OCR result (first 500 chars): {lark_text[:500]}")
                except Exception as e:
                    log.append(f"Lark OCR FAILED: {type(e).__name__}: {e}")
                
                os.unlink(tmp_path)
        except Exception as e:
            log.append(f"pymupdf FAILED: {type(e).__name__}: {e}")

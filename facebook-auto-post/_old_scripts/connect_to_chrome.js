...
async function main() {
  console.log('===========================================');
  console.log(' Facebook 自動發文 - v10.0 Remote Connect');
  console.log('===========================================\n');
  ...
  try {
    // 1. 連接到 Chrome
    console.log('[1/6] 連接到 Chrome Remote Debug...');
    ...
    // 2. 獲取頁面
    console.log('[2/6] 獲取頁面...');
    let pages = await browser.pages();
    
    let page;
    if (pages.length === 0) {
      console.log('沒有頁面，創建新頁面...');
    ...
    if (!page.url().includes('facebook.com') || page.url().includes('login')) {
      console.log('⚠️ 請確保在 Chrome 中已登入 Facebook\n');
      console.log('按 Enter 繼續（或等待 60 秒）...\n');
    ...
    for (let i = 0; i < CONFIG.groups.length; i++) {
      ...
      try {
        ...
        if (!editableDiv) {
          ...
          await page.screenshot({ path: `/tmp/v10_no_editor_${i}.png` });
          continue;
        }
        console.log('✅ 找到可編輯區域');
        ...
        // 截圖
        await page.screenshot({ path: `/tmp/v10_result_${i}_${group.id.substring(0, 8)}.png` });
        console.log(`截圖: /tmp/v10_result_${i}_${group.id.substring(0, 8)}.png`);
        ...
        await page.screenshot({ path: `/tmp/v10_error_${i}.png` }).catch(() => {});
      }
    }

...
async function clickPostBox(page) {
  const selectors = [
...
async function main() {
  console.log('===========================================');
  console.log(' Facebook 自動發文測試 - v4.0 Stealth');
  console.log('===========================================\n');
    ...
    for (let i = 0; i < CONFIG.groups.length; i++) {
      ...
      try {
        ...
        if (!editableDiv) {
          ...
          await page.screenshot({ path: `/tmp/stealth_no_editor_${i}.png` });
          continue;
        }
        console.log('✅ 找到可編輯區域');
        ...
        await page.screenshot({ 
          path: `/tmp/stealth_result_${i}_${group.id.substring(0, 8)}.png` 
        });
        console.log(`截圖: /tmp/stealth_result_${i}_${group.id.substring(0, 8)}.png`);

        // 返回主頁刷新 session
        console.log('返回主頁刷新 session...');
        ...
        await page.screenshot({ path: `/tmp/stealth_error_${i}.png` }).catch(() => {});
      }
    }

    console.log('\n========== 測試完成 ==========\n');
    console.log('[5/6] ✅ Stealth 測試完成\n');

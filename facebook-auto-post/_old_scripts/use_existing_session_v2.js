...
async function postToGroup(page, group, templateNum) {
  ...
  try {
    ...
    const composerSelectors = [
      // 使用 text locator
      { type: 'text', value: '写点什么...' },
      { type: 'text', value: '说点什么...' },
      // aria-label
      'div[aria-label*="写"]',
      'div[aria-label*="说"]',
      // 浮動按鈕
    ...
    if (!clicked) {
      // 備用：直接用坐標點擊頁面頂部的輸入框區域
      console.log('[DEBUG] 嘗試坐標點擊頂部區域');
    ...
    const inputSelectors = [
      'div[contenteditable="true"][aria-label*="写"]',
      'div[contenteditable="true"][aria-label*="说"]',
    ...
    for (const selector of inputSelectors) {
      ...
      if (el) {
        ...
        if (visible) {
          inputEl = el;
          console.log(`[DEBUG] 找到輸入框: ${selector}`);
          break;
        }
      }
    }
...
async function main() {
  console.log('========== 使用已登入 Session 發文 (v2) ==========');
